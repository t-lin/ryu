...
class RateControl (object):
    ...
    def check_if_over_rate (self, dpid, in_port, src_mac, ts):

        curr_rate_info = self.get_rate_info (dpid, in_port, src_mac)
    ...
    def get_rate_info (self, dpid, in_port, src_mac):
        dpid_dict = self.rate_list.setdefault(dpid, {})
        inport_dict = dpid_dict.setdefault(in_port, {})
        if src_mac not in inport_dict:
            smac_dict = inport_dict.setdefault(src_mac, {})
            dp_port_list = self.mac_dict.setdefault(src_mac, [])
            dp_port_list.append((dpid, in_port))
        else:
            smac_dict = inport_dict.setdefault(src_mac, {})
        return smac_dict

    def mac_deleted(self, mac):
        try:
            for (dpid, in_port) in self.mac_dict[mac]:
                try:
                    self.rate_list[dpid][in_port].pop(mac, None)
                    if len(self.rate_list[dpid][in_port]) == 0:
                        self.rate_list[dpid].pop(in_port, None)
                    if len(self.rate_list[dpid]) == 0:
                        self.rate_list.pop(dpid, None)
                except:
                    pass
            self.mac_dict.pop(mac, None)
        except:
            pass
